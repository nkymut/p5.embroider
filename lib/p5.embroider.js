(function (factory) {
  ...
  (function (global) {
    ...
    function overrideRectFunction() {
      ...
      window.rect = function (x, y, w, h, ...cornerRs) {
        if (_recording) {
          ...
          const pathPoints = [];
          const arcDetail = Math.max(5, Math.ceil(Math.max(tl, tr, br, bl) / 2));
    ...
    function overrideArcFunction() {
      ...
      window.arc = function(x, y, w, h, start, stop, mode) {
        if (_recording) {
          ...
          const numSteps = Math.max(12, Math.ceil(Math.abs(stop - start) * Math.max(w, h) / (_embroiderySettings.stitchLength * 2)));
          const pathPoints = [];
          for (let i = 0; i <= numSteps; i++) {
          ...
          // If mode is PIE, close to center
          if (mode === window.PIE) {
            pathPoints.push({ x, y });
            pathPoints.push(pathPoints[0]);
          }
          // Fill
          if (_doFill && (mode === window.PIE || mode === window.CHORD)) {
            let fillStitches = [];
            switch (_currentFillMode) {
              case FILL_MODE.TATAMI:
                fillStitches = createTatamiFillFromPath(pathPoints, _fillSettings);
    ...
    function zigzagStitch(x1, y1, x2, y2, stitchSettings) {
      ...
      if (distance === 0 || distance < 0.001) {
        ...
        console.log("Zero distance detected in zigzagStitch, returning single point");
        return [{
          x: x1,
          y: y1,
        }];
      ...
      console.log("Generated zigzag stitches:", stitches);
      return stitches;
    }
    ...
    function straightLineStitch(x1, y1, x2, y2, stitchSettings = _embroiderySettings) {
      ...
      console.log("Generated straight line stitches:", stitches);
      return stitches;
    }
    ...
    p5embroidery.exportDST = function (filename = "embroideryPattern.dst") {
      ...
      for (let threadIndex = 0; threadIndex < _stitchData.threads.length; threadIndex++) {
        ...
        if (currentThreadIndex !== -1 && threadIndex !== currentThreadIndex && points.length > 0) {
          ...
          console.log("Color change at:", lastPoint.x, lastPoint.y);
        }
        ...
        for (const run of thread.runs) {
          ...
          if (run.length === 1 && run[0].command === "trim") {
            ...
            if (run[0].x == null || run[0].y == null || 
              ...
              console.warn("Skipping invalid trim command with null/NaN coordinates:", run[0]);
              continue;
            }
          ...
          for (const stitch of run) {
            ...
            if (stitch.x == null || stitch.y == null || 
              ...
              console.warn("Skipping invalid stitch with null/NaN coordinates:", stitch);
              continue;
            }
    ...
    p5embroidery.trimThread = function (threadIndex = _strokeThreadIndex) {
      if (_recording) {
        ...
        console.log("Adding trim at position:", currentX, currentY);
  ...
  function straightLineStitchFromPath(pathPoints, stitchSettings = _embroiderySettings) {
    ...
    console.log("Generated straight line path stitches:", result);
    return result;
  }
