(function(global) {
  ...
  let _dstWriter = new DSTWriter(); // Add DSTWriter instance
  ...
  p5embroidery.endRecord = function(format = 'dst') {
    _recording = false;
    restoreP5Functions();
  ...
  p5embroidery.exportDST = function(filename = 'embroideryPattern.dst') {
    ...
    if (typeof DSTWriter === 'undefined') {
      console.error('DSTWriter is not defined. Make sure p5-tajima-dst-writer.js is loaded.');
      return;
    }
    
    _dstWriter.saveDST(points, "EmbroideryPattern", filename);
  }
