(function (factory) {
  ...
  class DSTWriter {
    ...
    generateDST(points, title) {
      ...
      for (let i = 0; i < transformedPoints.length; i++) {
        ...
        // Handle thread trim — DST signals a trim via consecutive zero-displacement JUMP records.
        // Most machines (Brother, Tajima) interpret 3+ consecutive jumps as "trim".
        if (point.trim) {
          {
          ...
          // Move to the trim position first
          this.move(point.x, point.y, DSTWriter.JUMP);

          // Emit 3 zero-displacement JUMP records as the trim signal
          for (let t = 0; t < 3; t++) {
            this.data.push(...this.encodeRecord(0, 0, DSTWriter.JUMP));
            this.stitchCount++;
          }

          // Next point after trim must also be a JUMP (needle repositioning, no thread)
        ...
        let flag;
        if (i === 0 || point.jump || needJumpAfterTrim) {
          flag = DSTWriter.JUMP;
  ...
  class SVGWriter {
    ...
    drawEmbroideryPatterns(stitchData) {
      ...
      for (let i = 0; i < threadsToDraw.length; i++) {
        ...
        this.options.threads ? this.options.threads[i] : i;
  ...
  class PESWriter {
    ...
    findColor(color) {
      ...
      const std = [
        0x1a0a94, 0x0f75ff, 0x00934c, 0xbabdfe, 0xec0000, 0xe4995a, 0xcc48ab, 0xfdc4fa, 0xdd84cd, 0x6bd38a, 0xe4a945,
        0xffbd42, 0xffe600, 0x6cd900, 0xc1a941, 0xb5ad97, 0xba9c5f, 0xfaf59e, 0x808080, 0x000000, 0x001cdf, 0xdf00b8,
        0x626262, 0x69260d, 0xff0060, 0xbf8200, 0xf39178, 0xff6805, 0xf0f0f0, 0xc832cd, 0xb0bf9b, 0x65bfeb, 0xffba04,
        0xfff06c, 0xfeca15, 0xf38101, 0x37a923, 0x23465f, 0xa6a695, 0xcebfa6, 0x96aa02, 0xffe3c6, 0xff99d7, 0x007004,
        0xedccfb, 0xc089d8, 0xe7d9b4, 0xe90e86, 0xcf6829, 0x408615, 0xdb1797, 0xffa704, 0xb9ffff, 0x228927, 0xb612cd,
        0x00aa00, 0xfea9dc, 0xfed510, 0x0097df, 0xffff84, 0xcfe774, 0xffc864, 0xffc8c8, 0xffc8c8,
      ];
    ...
    writePecGraphics() {
      this.writeBytes([
        ,
      ]);
    }
    ...
    generatePES(points, title) {
      ...
      const centerX = border.width / 2;
      const centerY = border.height / 2;
      const offsetX = border.left + centerX;
      const offsetY = border.top + centerY;
  ...
  function drawPreviewControls(options = {}) {
    const { showSlider = true, showResetButton = true } = options;
    ...
    if (showSlider) {
      ...
      const t = Math.max(
        0,
        Math.min(1, (_previewState.scale - _previewState.minScale) / (_previewState.maxScale - _previewState.minScale)),
      );
  ...
  (function (global) {
    ...
    function drawTrimAnnotations() {
      ...
      for (const pt of _trimAnnotations) {
        ...
        let lineLength = 10;
        let endX = mmToPixel$1(pt.x) + lineLength;
        let endY = mmToPixel$1(pt.y) - lineLength;
        _originalStrokeFunc.call(_p5Instance, 255, 0, 0);
        _originalStrokeWeightFunc.call(_p5Instance, 0.5);
        _originalLineFunc.call(_p5Instance, mmToPixel$1(pt.x), mmToPixel$1(pt.y), endX, endY);
        _p5Instance.push();
      ...
      // If we have a stroke weight, use the appropriate path-based function
        ...
        switch (strokeSettings.strokeMode) {
          ...
          case STROKE_MODE.VERTEX:
            return vertexStitchFromPath(pathPoints);
          default:
      ...
      window.text = function (str, x, y, maxWidth, maxHeight) {
        if (_recording) {
          ...
          if (!(font && font.font)) {
            ...
            if (_drawMode === "p5") {
              push();
              textSize(mmToPixel$1(fontSize));
              _originalTextFunc.call(
                _p5Instance,
                str,
                mmToPixel$1(x),
                mmToPixel$1(y),
                mmToPixel$1(maxWidth),
                mmToPixel$1(maxHeight),
              );
              pop();
            }
            return;
          ...
          push();
          textSize(mmToPixel$1(fontSize));
          _originalTextFunc.call(_p5Instance, str, mmToPixel$1(x), mmToPixel$1(y), mmToPixel$1(maxWidth), mmToPixel$1(maxHeight));
          pop();
        }
      };
    }
    ...
    function convertLineToStitches(x1, y1, x2, y2, stitchSettings = _embroiderySettings) {
      ...
      if (stitchSettings.strokeWeight > 0) {
        switch (_currentStrokeMode) {
          ...
          case STROKE_MODE.VERTEX:
            return vertexStitch(x1, y1, x2, y2);
          default:
            return straightLineStitch(x1, y1, x2, y2, stitchSettings);
        }
      } else {
    ...
    p5embroidery.drawSVGPart = function (part, options = {}) {
      ...
      if (offsetX !== 0 || offsetY !== 0) {
        if (typeof window.translate === "function") {
          window.translate(mmToPixel$1(offsetX), mmToPixel$1(offsetY));
        }
      }
      if (partScale !== 1) {
          ...
          case "circle":
            if (typeof sp.cx === "number" && typeof sp.cy === "number" && typeof sp.r === "number") {
              if (typeof window.circle === "function") {
                window.circle(mmToPixel$1(sp.cx), mmToPixel$1(sp.cy), mmToPixel$1(sp.r * 2));
                drewPrimitive = true;
              }
            }
            break;
          case "rect":
            if (typeof sp.x === "number" && typeof sp.y === "number" && typeof sp.w === "number" && typeof sp.h === "number") {
              if (typeof window.rect === "function") {
                window.rect(mmToPixel$1(sp.x), mmToPixel$1(sp.y), mmToPixel$1(sp.w), mmToPixel$1(sp.h));
                drewPrimitive = true;
              }
            }
            break;
          case "ellipse":
            if (typeof sp.cx === "number" && typeof sp.cy === "number" && typeof sp.rx === "number" && typeof sp.ry === "number") {
              if (typeof window.ellipse === "function") {
                window.ellipse(mmToPixel$1(sp.cx), mmToPixel$1(sp.cy), mmToPixel$1(sp.rx * 2), mmToPixel$1(sp.ry * 2));
                drewPrimitive = true;
              }
            }
            break;
          case "line":
            if (typeof sp.x1 === "number" && typeof sp.y1 === "number" && typeof sp.x2 === "number" && typeof sp.y2 === "number") {
              if (typeof window.line === "function") {
                window.line(mmToPixel$1(sp.x1), mmToPixel$1(sp.y1), mmToPixel$1(sp.x2), mmToPixel$1(sp.y2));
                drewPrimitive = true;
              }
            }
            break;
          ...
          for (const shape of shapes) {
            ...
            for (const pt of shape.outer) {
              if (typeof window.vertex === "function") {
                window.vertex(mmToPixel$1(pt.x), mmToPixel$1(pt.y));
              }
            }
