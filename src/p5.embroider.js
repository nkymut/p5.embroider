    ...
    for (let i = 1; i < bezierPoints.length; i++) {
      ...
      if (_drawMode === "p5" && i === 1) {
        _originalBezierVertexFunc.call(
          _p5Instance,
          mmToPixel(cp1.x), mmToPixel(cp1.y),
          mmToPixel(cp2.x), mmToPixel(cp2.y),
          mmToPixel(endPt.x), mmToPixel(endPt.y),
        );
      }
    }
    _isBezier = true;
  ...
  function overrideQuadraticVertexFunction() {
    ...
    fn.quadraticVertex = function (cx, cy, x3, y3) {
      ...
      if (_recording) {
        ...
        _originalQuadraticVertexFunc.apply(this, [mmToPixel(cx), mmToPixel(cy), mmToPixel(x3), mmToPixel(y3)]);
      }
    };
  }
