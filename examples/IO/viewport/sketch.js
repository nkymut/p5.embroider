...
function draw() {
  ...
  text(
    `Mouse in world: (${pixelToMm((mouseX - state.centerX - state.panX) / state.scale + state.centerX).toFixed(1)}mm, ${pixelToMm((mouseY - state.centerY - state.panY) / state.scale + state.centerY).toFixed(1)}mm)`,
    10,
    height - 30,
  );
